    ...
    <div class="page-title">
        ...
        <div class="title_right">
            <div class="col-md-5 col-sm-5 col-xs-12 form-group pull-right top_search text-right">
                ...
                <h4><a href="{{ route('members.import') }}">Import Excel</a></h4>
            </div>
        </div>
    </div>
        ...
        <div class="x_panel">
            <div class="x_content">
                <div class="row">
                    <div class="col-sm-12">
                        @if ($status == 'active')
                            <a href="{{ route('member.excel') }}" class="btn btn-danger float-right">Download Excel
                                Sheet</a>
                        @endif
                                    ...
                                    @foreach ($rows as $row)
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                @if ($status == 'pending')
                                                    <a href="{{ url('/admin/members?id=' . $row->id . '&status=approve') }}"
                                            ...
                                            <td>
                                                ...
                                                <a href="{{ route('members.edit', ['member' => $row->id]) }}"
                                                    class="btn btn-info">Edit</a>
                                                <form method="post"
                                                    action="{{ route('members.destroy', ['member' => $row->id]) }}">
                                                    @csrf
                                                    @method('delete')
                                                    <button type="submit" class="btn btn-danger">Delete</button>
                                                </form>
                                                @if ($status == 'active')
                                                    <a href="{{ url('/admin/members?id=' . $row->id . '&status=deathapply') }}"
